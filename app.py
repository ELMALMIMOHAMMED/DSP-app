...
def calculate_fft():
    ...
    func = data['function']  # Fonction sous forme de chaîne

    # Domaine de définition de la fonction
    x = np.linspace(0, 10, 1000)
    
    try:
        # Créer un contexte où 't' et les fonctions mathématiques sont définies
        context = {
            'sin': sin,
            'cos': cos,
            'pi': pi
        ...
        # Evaluer la fonction pour chaque valeur de x
        # Utilisation de t dans le contexte
        y = [eval(func, {"__builtins__": None}, {**context, 't': xi}) for xi in x]
